...
function ColorForm(props: {
  ...
  return (
    <form id="form-colors" autoComplete="off">
      <p>
        <b>Showing Colors for: </b>
        {file.name}
      </p>
      <div className="analysis_result">{colorInputsArray}</div>
      <div className="status-wrapper">
        <Alert
          ...
          <Spinner
            ...
            role="status"
            hidden={
              formState.alert.msg !== "Waiting for Server response..."
            }></Spinner>
          {formState.errors.size !== 0
            ? "You submitted invalid new colors for: " +
              Array.from(formState.errors).join(", ")
            : formState.alert.msg}
        </Alert>
      </div>
      <Button
                  ...
                  setFormState((state) =>
                    update(state, {
                      alert: {
                        ...
                        msg: {
                          $set:
                            "Request came back with the following error message: " +
                            response.message,
                        },
                      },
                    })
