...
> {
  ...
  readonly componentsByProgress: ComponentMap = {
    [Progress.initial]: (
      <FileUpload
        showColors={(colors: Array<Array<String>>, file: File) => {
          this.setState({
            colors: colors,
            progress: Progress.submittedPDF,
            file: file,
          })
        }}></FileUpload>
    ),
    [Progress.submittedPDF]: (
      <ColorForm
        ...
        handleChange={(b64_in: string) => {
          this.setState({ b64: b64_in, progress: Progress.submittedColors })
        }}></ColorForm>
    ),
    [Progress.submittedColors]: (
      <Download
        b64={() => {
          return this.state.b64
        }}
        oldFileName={() => {
    ...
    return (
      <div>
        <ProgressMeter
          ...
          progress={this.state.progress}></ProgressMeter>
